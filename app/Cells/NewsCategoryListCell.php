...
class NewsCategoryListCell extends Cell
{
	public $stt = 0;
	public $tr = '';
    ...
    public function getTrTable () {
        ...
        foreach ($level_1 as $item) {
        	...
        	$this->tr .= '<td><a href="/admin/newscat/edit/'. $item['id'] .'">'.esc($item['title']).'</a></td>';
        	$this->tr .= '<td>'.getInfoUser($item['creator_id'])['username'].'</td>';
        	$this->tr .= '<td>
	                        <a href="/admin/newscat/edit/'. $item['id'] .'">Edit</a> |
	                        <form action="/admin/newscat/delete/'. $item['id'] .'" method="post" style="display: inline;">
	                          '. csrf_field() .'
    ...
    public function level_child ($id, $level) {
    	...
    	foreach ($level_next as $item) {
        	...
        	$this->tr .= '<tr>';
        	$this->tr .= '<td>'.$this->stt.'</td>';
        	$this->tr .= '<td><a href="/admin/newscat/edit/'. $item['id'] .'">'.$tab.' '.esc($item['title']).'</a></td>';
        	$this->tr .= '<td>'.getInfoUser($item['creator_id'])['username'].'</td>';
        	$this->tr .= '<td>
	                        <a href="/admin/newscat/edit/'. $item['id'] .'">Edit</a> |
	                        <form action="/admin/newscat/delete/'. $item['id'] .'" method="post" style="display: inline;">
